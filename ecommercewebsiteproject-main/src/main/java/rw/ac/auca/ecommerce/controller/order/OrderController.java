...
public class OrderController {
    ...
    public String createOrderForm(@PathVariable UUID id, Model model) {
        Product product = productService.findProductByIdAndState(id, true);

        if (product == null) {
            return "redirect:/products"; // or an error page
        }

        Order order = new Order(); // empty order, no direct product

        model.addAttribute("order", order);        // For order form binding
        model.addAttribute("product", product);    // Product info for display
    ...
    public String submitOrder(@RequestParam UUID productId,
        ...
        // For now, pick the first active customer as the buyer
        Customer customer = customerService.findCustomersByState(true).stream().findFirst().orElse(null);
        if (customer == null) {
            // No customer found - handle error
            return "redirect:/order/create?error=customerNotFound";
        }
    ...
    // New save order endpoint accepting Order model attribute
